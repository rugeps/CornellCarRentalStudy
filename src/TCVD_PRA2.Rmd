...
numeric_features_names = c('rating', 'renterTripsTaken', 'reviewCount', 'rate.daily', 'age', 'income', 'frequency')
```
...
ggplot(data=ds, aes(x = rent)) + geom_bar(aes(y = (..count..)/sum(..count..))) + geom_text(aes(y = ((..count..)/sum(..count..)), label = scales::percent((..count..)/sum(..count..))), stat = "count", vjust = -0.5) + scale_y_continuous(labels = scales::percent, limits = c(0,1)) + xlab("Rent") + ylab("FreqÃ¼Ã¨ncia")
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Generem el model d'arbre
model_cart_frequency <- rpart(frequency ~ fuelType + location.city + location.state + vehicle.make + vehicle.model + vehicle.type, method="anova", data=train)
